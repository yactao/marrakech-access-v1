...
// Images d'ambiance par quartier (pour enrichir la galerie)
...
export default function PropertyDetailPage() {
  ...
  const [property, setPropertyData] = useState<any>(null);
  ...
  useEffect(() => {
    if (!slug) return;
    setLoading(true);
    Promise.all([api.get(`/properties/${slug}`), api.get('/extras')])
      .then(([propRes, extrasRes]) => {
        setPropertyData(propRes.data.property);
    ...
    return (
      <main className="min-h-screen pt-24 pb-16 px-4 bg-dark flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="font-playfair text-2xl text-white/60">{error}</h1>
  ...
  // Construire la galerie : coverPhoto + photos du bien + photos du quartier
          ...
          {allPhotos.length > 1 && (
            <>
              <button className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white text-xl z-10"
                onClick={(e) => { e.stopPropagation(); setActivePhoto((prev) => (prev - 1 + allPhotos.length) % allPhotos.length); }}>
                ‹
              </button>
              <button className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white text-xl z-10"
                onClick={(e) => { e.stopPropagation(); setActivePhoto((prev) => (prev + 1) % allPhotos.length); }}>
                ›
              </button>
            </>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {property.reviews && property.reviews.length > 0 && (
              <div>
                <h2 className="font-playfair text-lg font-semibold text-gold mb-3">Avis ({property.reviewsCount})</h2>
                <div className="space-y-4">
                  {property.reviews.map((review: any) => (
                    <div key={review.id} className="p-4 rounded-lg bg-dark-light border border-white/5">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-gold/20 flex items-center justify-center text-gold text-xs font-bold">{review.author.firstName[0]}</div>
