...
async function searchProperties(args: {
  ...
  const properties = await prisma.property.findMany({
    ...
    select: {
      ...
      priceLowSeason: true, priceHighSeason: true, cleaningFee: true,
      currency: true, shortDesc: true, amenities: true, minNights: true,
      coverPhoto: true,
    },
  ...
  return {
    ...
    properties: properties.map((p) => ({
      id: p.id, nom: p.name, slug: p.slug, type: p.type, quartier: p.district,
      ...
      priceLowSeason: Number(p.priceLowSeason),
      priceHighSeason: Number(p.priceHighSeason),
  ...
  for (const where of whereVariants) {
    properties = await prisma.property.findMany({
      ...
      select: {
        ...
        priceHighSeason: true, cleaningFee: true, currency: true,
        shortDesc: true, minNights: true, coverPhoto: true,
      },
      orderBy: { priceLowSeason: 'asc' },
      take: 5,
    });
...
async function getUpsellSuggestions(args: {
  ...
  const extras = await prisma.extra.findMany({
    ...
    select: {
      ...
      price: true, priceUnit: true, duration: true, maxPersons: true, photo: true,
    },
    orderBy: { sortOrder: 'asc' },
    take: 3,
  });
